import {
  ...
  interface MomoTransChartProps {
    data: Transaction[];
  }
  
  export function MomoTransChart({ data }: MomoTransChartProps) {
    console.log("MomoTransChart:", data);
    ...
    return (
      <div className="bg-white shadow-lg p-4 pt-6 rounded-md">
        <h3 className="text-center mb-4">Transaction Momo Summary (Last 6 Months)</h3>
        <ResponsiveContainer width="100%" aspect={getAspect()}>
          <ChartContainer config={{ waitingForm: { color: "#90EE90" } }}>
            <BarChart width={900} height={450} data={processedData}>
              <CartesianGrid strokeDasharray="2" strokeWidth={"3"} />
              <XAxis dataKey="month" />
              <YAxis />
              <Tooltip />
              <Bar dataKey="completed" fill="#90EE90" /> {/* Màu xanh lá đậm */}
            </BarChart>
          </ChartContainer>
        </ResponsiveContainer>
      </div>
